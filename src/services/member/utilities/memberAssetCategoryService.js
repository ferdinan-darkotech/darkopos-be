...
let table = db.tbl_member_asset_category

const Fields = ['id', 'categoryCode', 'categoryName', 'active', 'createdAt', 'createdBy', 'updatedAt', 'updatedBy']
...
export function getData (query, pagination) {
  ...
  let querying = []
  if (other['q']) {
    const checking = ['id', 'categoryCode', 'categoryName']
    for (let key in Fields) {
      const id = Object.assign(Fields)[key]
      if (id === 'categoryCode' || id === 'categoryName') {
        let obj = {}
