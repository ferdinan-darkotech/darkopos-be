...
const table = tb.tbl_notification_group
const view = vw.vwi_notification_group_001
...
const mainFields = [
  'code', 'name', 'timerId', 'autoApprove',
  'createdBy', 'createdAt', 'updatedBy', 'updatedAt'
]
const mainViewFields = [
  'code', 'name', 'timerId', 'timerName', 'timerPattern', 'autoApprove',
  'createdBy', 'createdAt', 'updatedBy', 'updatedAt'
]

export async function srvGetNotificationGroups (query, filter = false) {
...
export async function srvGetNotificationGroupByCode (code, query = {}) {
  let { pageSize, page, order, q, ...other } = query
...
export async function srvGetNotificationGroupById (id) {
  return table.findOne({
...
export function srvNotificationGroupExist (code) {
  return srvGetNotificationGroupByCode(code).then(data => {
    if (data == null) {
      return false
    }
    else {
...
export async function srvCreateNotificationGroup (data, createdBy, next) {
  return table.create({
    code: data.code,
    name: data.name,
    ...
    other.code = 'ZSNG-00001'
    next(new ApiError(400, other, err))
  })
}

export async function srvUpdateNotificationGroup (data, updatedBy, next) {
  return table.update({
    name: data.name,
    timerId: data.timerId,
    autoApprove: data.autoApprove,
    ...
    const { parent, original, sql, ...other } = JSON.parse(JSON.stringify(err))
    other.code = 'ZSNG-00002'
    next(new ApiError(400, other, err))
  })
}

export function srvDeleteNotificationGroup (code, next) {
  return table.destroy({
    where: { code }
  }).catch(err => {
    const { parent, original, sql, ...other } = JSON.parse(JSON.stringify(err))
    other.code = 'ZSNG-00003'
