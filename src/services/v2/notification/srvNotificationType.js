...
const table = tb.tbl_notification_type
...
export async function srvGetNotificationTypes (query, filter = false) {
...
export async function srvGetNotificationTypeByCode (code, query = {}) {
  let { pageSize, page, order, q, ...other } = query
...
export async function srvGetNotificationTypeById (id) {
  return table.findOne({
...
export function srvNotificationTypeExist (code) {
  return srvGetNotificationTypeByCode(code).then(data => {
    if (data == null) {
      return false
    }
    else {
...
export async function srvCreateNotificationType (data, createdBy, next) {
  return table.create({
    code: data.code,
    name: data.name,
    createdBy: createdBy,
    ...
    other.code = 'ZSNT-00001'
    next(new ApiError(400, other, err))
  })
}

export async function srvUpdateNotificationType (data, updatedBy, next) {
  return table.update({
    name: data.name,
    updatedBy: updatedBy
  }, {
    ...
    const { parent, original, sql, ...other } = JSON.parse(JSON.stringify(err))
    other.code = 'ZSNT-00002'
    next(new ApiError(400, other, err))
  })
}

export function srvDeleteNotificationType (code, next) {
  return table.destroy({
    where: { code }
  }).catch(err => {
    const { parent, original, sql, ...other } = JSON.parse(JSON.stringify(err))
    other.code = 'ZSNT-00003'
