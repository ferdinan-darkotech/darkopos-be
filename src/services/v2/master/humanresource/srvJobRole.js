...
const table = tb.tbl_job_position
...
export async function srvGetJobRoles (query, filter = false) {
...
export async function srvGetJobRoleByCode (jobRoleCode, query = {}) {
  let { pageSize, page, order, q, ...other } = query
  let attributes = minFields01
  let where = { positionCode: jobRoleCode }
...
export async function srvGetJobRoleById (id) {
  return table.findOne({
...
export function srvJobRoleExist (positionCode) {
  return srvGetJobRoleByCode(positionCode).then(data => {
    if (data == null) {
      return false
    }
    else {
...
export async function srvCreateJobRole (data, createdBy, next) {
  return table.create({
    ...
    other.code = 'ZSJR-00001'
    next(new ApiError(400, other, err))
  })
}

export async function srvUpdateJobRole (data, updatedBy, next) {
  return table.update({
    positionName: data.positionName,
    updatedBy: updatedBy
  }, {
    ...
    const { parent, original, sql, ...other } = JSON.parse(JSON.stringify(err))
    other.code = 'ZSJR-00002'
    next(new ApiError(400, other, err))
  })
}

export function srvDeleteJobRole (positionCode, next) {
  return table.destroy({
    where: { positionCode }
  }).catch(err => {
    const { parent, original, sql, ...other } = JSON.parse(JSON.stringify(err))
    other.code = 'ZSJR-00003'
