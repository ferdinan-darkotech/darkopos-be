...
} from '../../services/master/bundlingRewardService'
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getStocks-BundlingReward: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  console.log('Requesting-insertData-BundlingReward: ' + req.url + ' ...')
  const data = req.body
  const userLogIn = extractTokenProfile(req)
  dataExistsCode(data.bundleId).then(exists => {
    if (!exists) {
      next(new ApiError(422, `Cannot insert header does'nt exists`))
    } else {
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDataBundlingReward: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
