...
import {
  getDataId, getData, countData, insertData, insertBulkData, updateData, deleteData,
  dataExists, dataExistsCode, dataExistsCodeLooping
} from '../../services/product/stockSpecificationService'
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataSpecification: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  ...
  dataExistsCode(data).then(exists => {
    if (!exists) {
      ...
      }).catch(err => next(new ApiError(422, `Couldn't create specification.`, err)))
    } else {
...
// Create a new data
exports.insertBulkData = function (req, res, next) {
  console.log('Requesting-insertBulkData: ' + req.url + ' ...')
  const data = req.body.data
  const userLogIn = extractTokenProfile(req)
  const productId = data.map(x => x.productId)
  const specificationId = data.map(x => x.specificationId)
  dataExistsCodeLooping(productId, specificationId).then(exists => {
    if (!exists) {
      return insertBulkData(data, userLogIn.userid, next).then((created) => {
        let jsonObj = {
          success: true,
          message: `Data created`,
        }
      ...
      next(new ApiError(422, `Record ${(data[0] || {}).name} already exists.`))
    }
  }).catch(err => next(new ApiError(422, `Couldn't create specification.`, err)))
}
