...
import {
  getDataId, getData, countData, insertData, closingData, deleteData,
...
exports.getDataId = function (req, res, next) {
  ...
  getDataId(id, req.query.relationship).then((data) => {
...
//Update a Data
exports.closingData = function (req, res, next) {
  console.log('Requesting-closingData: ' + req.url + ' ...')
  const id = req.params.id
  let data = req.body.data
  const userLogIn = extractTokenProfile(req)
  dataExists(id).then(exists => {
    if (exists) {
      return closingData(id, data, userLogIn.userid).then((updated) => {
        if (updated) {
          let jsonObj = {
            success: true,
