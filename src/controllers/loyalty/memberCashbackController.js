...
} from '../../services/loyalty/memberCashbackService'
import { extractTokenProfile } from '../../services/v1/securityService'

// Retrieve list a row
exports.getDataId = function (req, res, next) {
  console.log('Requesting-getDataIdLoyaltyMember: ' + req.url + ' ...')
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getLoyaltyMember: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  ...
  dataExistsCode(data.accountCode).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record Active Already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create stock.`, err)))
}
