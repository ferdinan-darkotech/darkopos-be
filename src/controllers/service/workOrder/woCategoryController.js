...
} from '../../../services/service/workorder/woCategoryService'
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataWo03: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  ...
  dataExistsCode(data.productCategoryId).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record ${data.productCategoryId} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create wo.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDataWOCategory: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
