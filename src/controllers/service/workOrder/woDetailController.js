...
import {
  ...
  dataExists, dataExistsCode, srvGetLastFieldsChecked
} from '../../../services/service/workorder/woDetailService'
import { extractTokenProfile } from '../../../services/v1/securityService'



// New syntax 2020-10-19 : to fetch last data service
exports.ctlGetLastFieldsChecked = function (req, res, next) {
  console.log('Requesting-ctlGetLastFieldsChecked: ' + req.url + ' ...')
  if(req.params.type === 'LCF') { // last-checked-fiedls
    return srvGetLastFieldsChecked(req.query.member, req.query.policeno).then((data) => {
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataWo05: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  ...
  dataExistsCode(data).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record ${data.fieldId} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create wo.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDataWODetail: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
  dataExistsCode(data).then(existss => {
    if (!existss) {
