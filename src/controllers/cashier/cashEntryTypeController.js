...
import {
  getDataId, getData, countData, insertData, dataExists,
  updateData, deleteData, dataExistsCode
} from '../../services/cashier/cashEntryTypeService'
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataCashEntry: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  ...
  dataExistsCode(data.typeCode).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(404, `Record ${data.typeCode} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create stock.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateCashierEntryType: ' + req.url + ' ...')
  const id = req.body.id
  let data = req.body
  const userLogIn = extractTokenProfile(req)
  dataExists(id).then(exists => {
    if (exists) {
      return updateData(id, data, userLogIn.userid, next).then((updated) => {
        if (updated) {
          let jsonObj = {
            success: true,
  ...
  }).catch(err => next(new ApiError(404, `Couldn't find Data ${id}.`, err)))
}
...
exports.deleteData = function (req, res, next) {
  ...
  dataExists(id).then(exists => {
    if (exists) {
      ...
      next(new ApiError(422, `Data ${id} not exists.`))
    }
  }).catch(err => next(new ApiError(422, `Data ${id} not exists.`, err)))
}
