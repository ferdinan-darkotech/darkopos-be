...
} from '../../../services/member/utilities/memberAssetBrandService'
import { extractTokenProfile } from '../../../services/v1/securityService'

// Retrieve list a row
exports.getDataId = function (req, res, next) {
  console.log('Requesting-getDataIdAssetBrand: ' + req.url + ' ...')
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataAssetBrand: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  console.log('Requesting-insertDataAssetBrand: ' + req.url + ' ...')
  const data = req.body
  const userLogIn = extractTokenProfile(req)
  dataExistsCode(data.brandName).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record ${data.brandName} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create stock.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDatAssetBranda: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
...
exports.deleteData = function (req, res, next) {
  console.log('Requesting-deleteDataAssetBrand: ' + req.url + ' ...')
  const id = req.params.id
  dataExists(id).then(exists => {
    if (exists) {
