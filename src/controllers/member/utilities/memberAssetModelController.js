...
} from '../../../services/member/utilities/memberAssetModelService'
import { extractTokenProfile } from '../../../services/v1/securityService'

// Retrieve list a row
exports.getDataId = function (req, res, next) {
  console.log('Requesting-getDataIdAssetModel: ' + req.url + ' ...')
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataAssetModel: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  console.log('Requesting-insertDataAssetModel: ' + req.url + ' ...')
  const data = req.body
  ...
  dataExistsCode(data).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record ${data.modelName} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create stock.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDataAssetModel: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
...
exports.deleteData = function (req, res, next) {
  console.log('Requesting-deleteDataAssetModel: ' + req.url + ' ...')
  const id = req.params.id
  dataExists(id).then(exists => {
    if (exists) {
