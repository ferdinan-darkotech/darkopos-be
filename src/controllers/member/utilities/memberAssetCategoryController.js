...
} from '../../../services/member/utilities/memberAssetCategoryService'
import { extractTokenProfile } from '../../../services/v1/securityService'

// Retrieve list a row
exports.getDataId = function (req, res, next) {
  console.log('Requesting-getDataIdAssetCategory: ' + req.url + ' ...')
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataAssetCategory: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  console.log('Requesting-insertDataAssetCategory: ' + req.url + ' ...')
  const data = req.body
  const userLogIn = extractTokenProfile(req)
  dataExistsCode(data.CategoryName).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record ${data.CategoryName} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create stock.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDatAssetCategorya: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
...
exports.deleteData = function (req, res, next) {
  console.log('Requesting-deleteDataAssetCategory: ' + req.url + ' ...')
  const id = req.params.id
  dataExists(id).then(exists => {
    if (exists) {
