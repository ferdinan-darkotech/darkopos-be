...
} from '../../../services/member/utilities/memberAssetTypeService'
import { extractTokenProfile } from '../../../services/v1/securityService'

// Retrieve list a row
exports.getDataId = function (req, res, next) {
  console.log('Requesting-getDataIdAssetType: ' + req.url + ' ...')
...
exports.getData = function (req, res, next) {
  console.log('Requesting-getDataAssetType: ' + req.url + ' ...')
...
exports.insertData = function (req, res, next) {
  console.log('Requesting-insertDataAssetType: ' + req.url + ' ...')
  const data = req.body
  ...
  dataExistsCode(data).then(exists => {
    if (!exists) {
      ...
      next(new ApiError(422, `Record ${data.typeName} already exists.`))
    }
  }).catch(err => next(new ApiError(501, `Couldn't create stock.`, err)))
}
...
exports.updateData = function (req, res, next) {
  console.log('Requesting-updateDataAssetType: ' + req.url + ' ...')
  const id = req.params.id
  console.log('id', id)
  let data = req.body
  const userLogIn = extractTokenProfile(req)
...
exports.deleteData = function (req, res, next) {
  console.log('Requesting-deleteDataAssetType: ' + req.url + ' ...')
  const id = req.params.id
  dataExists(id).then(exists => {
    if (exists) {
